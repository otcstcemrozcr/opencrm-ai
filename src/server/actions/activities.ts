...
export async function completeActivityAction(formData: FormData): Promise<void> {
  ...
  if (done) {
    await uncompleteActivity(user.orgId, id);
  } else {
...
const standaloneSchema = z.object({
  ...
  notes: z.string().max(2000).optional(),
  dueAt: z.string().optional().or(z.literal("")),
  related: z.string().regex(/^(lead|opportunity|account|contact):[0-9a-f-]{36}$/i, "Pick a related record"),
});

export async function createActivityStandalone(
  _prev: FormState,
  formData: FormData
): Promise<FormState> {
  const user = await requireUser();
  ...
  const [relatedType, relatedId] = parsed.data.related.split(":") as [
